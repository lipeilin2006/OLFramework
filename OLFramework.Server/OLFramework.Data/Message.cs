...
namespace OLFramework.Data
{
	...
	{
		private static ConcurrentQueue<Message> messagePool = new();
		/// <summary>
		/// 路由
		/// </summary>
        ...
        private Message(string route, MessageType messageType, Dictionary<int, object>? values = null, Status status = Status.Success)
        {
            Route = route.Hash();
            MessageType = messageType;
            if (values == null)
